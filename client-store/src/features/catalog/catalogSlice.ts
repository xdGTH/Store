...
>("catalog/fetchProductsAsync", async (_, thunkAPI) => {
  ...
  try {
    ...
    return response.items; //response.data.items (equivalent to)
  } catch (error: any) {
...
export const fetchProductFiltersAsync = createAsyncThunk(
  "catalog/fetchProductFilterAsync",
  async (_, thunkAPI) => {
    try {
...
export const catalogSlice = createSlice({
  ...
  reducers: {
    setProductParams: (state, action) => {
      state.productsLoaded = false; // it will trigger the useEffect on the Catalog
      state.productParams = {
        ...state.productParams,
        ...action.payload,
        pageNumber: 1,
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchProductsAsync.fulfilled, (state, action) => {
      productsAdapter.setAll(state, action.payload); //setting all the product retrieved from API
      state.status = "idle";
